...
class ButtonPoseGenerator(Generator):
    """
    Generator for the ButtonPose dataset

    """
    def __init__(self,
                 dataset_base_path,
                 object_ids = {"2-slot": 1, "3-slot": 2, "mushroombutton": 3, "arrowbutton": 4, "redbutton": 5, "unknownbutton": 6}, #dictionary with the names and object ids of the dataset
                 image_extension = ".png",
                 shuffle_dataset = True,
                 symmetric_objects = {"2-slot", 1, "3-slot", 2, "mushroombutton", 3, "arrowbutton", 4, "redbutton", 5, "unknownbutton", 6}, #set with names and indices of symmetric objects
                 **kwargs):
        """
        Initializes a generator for the ScrewPose dataset, based on EfficientPose's occlusion generator
        Args:
            dataset_base_path: path to the dataset
            object_ids: Dictionary mapping the ScrewPose object names to their object id's
        ...
        #self.object_id = 0 #hardcored for Assembly
        
        #set the class and name dict for mapping each other
        self.class_to_name = {0 : "2-slot", 1 : "3-slot", 2 : "mushroombutton", 3 : "arrowbutton", 4 : "redbutton", 5 : "unknownbutton"}
        ...
        self.name_to_mask_value = {"2-slot": 42, "3-slot": 84, "mushroombutton": 126, "arrowbutton": 168, "redbutton": 210, "unknownbutton": 255}
        ...
        if not self.check_path(self.dataset_base_path) or not self.check_path(self.dataset_path) or not self.check_path(self.model_path):
            print("Dataset path not valid.")
            return None
        
        #get path containing infos about the valid examples. valid_poses subfolder needs to be copied manually in this dir
        self.class_to_valid_examples, self.name_to_valid_examples = self.parse_valid_examples(self.name_to_class)
        
        #get all train or test data examples from the dataset in the given split
        if not "train" in kwargs or kwargs["train"]:
            self.data_file = os.path.join(self.dataset_path, "train.txt")
        else:
            self.data_file = os.path.join(self.dataset_path, "test.txt")
        ...
        print("    Parsing ground truths...")
        self.gt_dict = self.parse_yaml(self.dataset_path)
        print("    Parsing information labels...")
        self.info_dict = self.parse_yaml(self.dataset_path, filename = "info.yml")
        print("    Parsing 3D models...")
        ...
        self.image_paths, self.mask_paths, self.annotations, self.infos = self.prepare_dataset(self.dataset_path, self.data_examples, self.gt_dict, self.info_dict, self.class_to_valid_examples)
    ...
    def parse_valid_examples(self, name_to_class_dict):
        ...
        for object_name in name_to_valid_examples.keys():
            ...
            valid_examples_path = os.path.join(self.dataset_path, "valid_poses")
    ...
    def prepare_dataset(self, dataset_path, data_examples, gt_dict, info_dict, class_to_valid_examples):
        """
       Prepares the Assembly dataset and converts the data from the Linemod format to the EfficientPose format
        Args:
            dataset_path: path to the Linemod subscene containing Occlusion (This is a hardcoded parameter in the init function)
        ...
        """
        all_images_path = os.path.join(dataset_path, "rgb")
       ...
       Prepares the annotations from the Linemod dataset format into the EfficientPose format
        ...
        for single_gt_list, info, mask_path in zip(gt_list, info_list, mask_paths):
            ...
            for i, gt in enumerate(single_gt_list):
                ...
                if not found_object:
                    print("\nWarning: Did not find object in mask in image " + str(j))
                    # print(mask_path)
                    # print(gt["obj_id"])
    ...
    def parse_yaml(self, dataset_path, filename = "gt.yml"):
        """
       Reads a yaml file
        Args:
            dataset_path: Path to the yaml file
...
if __name__ == "__main__":
    ...
    train_gen = ButtonPoseGenerator("/datasets/ScrewPose/", use_6DoF_augmentation = True, use_colorspace_augmentation = True)
